- 甲蛙2306项目学习笔记，中间件部分结合了黑马商城里的教学内容，还有一些是从其他地方整理的
...
> 注意幂等性问题，各种宕机问题，定时任务对账

>  甲蛙的项目中，数据库设计和缓存设计很随便，性能有很大问题；这里做了全新的设计，不过并没有代码实现，重点就看看怎么设计

在真实的12306中，使用了 **Pivotal Gemfire** 这个大型分布式内存数据库来存储数据，有点力大砖飞的感觉；我们这里还是用mysql + redis；
...
如果是电商项目，我们直接缓存一个数字，预扣减时堆redis里的数字做减值即可；

但是对于高铁购票，用户可以自由选择车程，因此票数是不确定的，不是说一辆车就一定有x张票；

在原本的项目中，教程里是直接随便选了一个数据作为大概的库存数去扣减，不妥；

### 缓存区间座位数 (√)

对于每日的每个车次的每个区间，我们在Redis中记录它的座位数，key 由 **日期、车次、座位类型、出发站、到达站** 共5项数据组成；

> 其实也可以用Hash，存key:座位类型:数量；不过每次操作只会处理一种座位类型，分开也挺好

```
key: stock:{date}:{train_code}:{seat_type}:{from_station}:{to_station}
value: integer (余票数量)
```

这样一来，我们还需要**重新设计数据库表**结构；

```sql
-- 和缓存结构对应的表，用于持久化、后续对账
CREATE TABLE train_seat_inventory (
  id BIGINT PRIMARY KEY,
  train_id BIGINT,           -- 列车ID
  from_station VARCHAR(20),  -- 起始站 (最小区间)
  to_station VARCHAR(20),    -- 到达站 (最小区间)
  seat_type INT,             -- 座位类型 (1:一等座, 2:二等座)
  remaining_seats INT,       -- 剩余座位数
  version INT                -- 乐观锁版本
);

-- 具体座位表，选座时使用
CREATE TABLE train_seat_bitmap (
  id BIGINT PRIMARY KEY, 						-- 主键，自增唯一标识每个座位记录
  train_id BIGINT NOT NULL, 					-- 列车ID
  date DATE NOT NULL, 							-- 发车日期
  carriage_number VARCHAR(16) NOT NULL,  		-- 车厢号
  seat_number VARCHAR(16) NOT NULL, 			-- 座位号
  seat_type INT NOT NULL, 						-- 座位类型 (1=一等座, 2=二等座)
  occupancy BIGINT UNSIGNED NOT NULL DEFAULT 0, -- 座位小区间占用位图(bitmask)，每个位代表一个区间是否已售
  UNIQUE KEY uq_seat (train_id, date, carriage_number, seat_number) 	-- 联合唯一索引
  INDEX idx_train_date (train_id, date) 								-- 列车ID+日期索引
);
```

## 车票查询

车票查询需要查询**车次**，然后查询车次的**余票**，最后统一成一个列表；

### Redis缓存

将查询的结果用JSON格式化后存储在redis中，key为：

```
ticket:{date}{from_station}:{to_station}
```

采用**逻辑过期**防缓存击穿，过期时间设置为5s；

采用**布隆过滤器**防缓存击穿；

### 车次查询

当用户购票时，选择 **日期、起点站、终点站**

查找符合条件的**车次**，基本逻辑如下（省略了日期）：
...
// 假设用户查询B→E的所有车次
// 找到所有经过B站和E站的列车
public List<String> findTrainsPassingStations(String fromStation, String toStation) {
    // 查询所有经过B站的列车
    List<String> trainsFrom = trainRouteMapper.findTrainsByStation(fromStation);

    // 查询所有经过E站的列车
    List<String> trainsTo = trainRouteMapper.findTrainsByStation(toStation);

    // 取交集：既经过B站又经过E站的列车
    trainsFrom.retainAll(trainsTo);

    // 进一步过滤：B站在E站之前
    return trainsFrom.stream()
        .filter(trainId -> isStationBefore(trainId, fromStation, toStation))
...
- 找到起点站和终点站之间的所有车站，获取列车的完整路线List
- 根据这些车站确定本次查询涉及的最小区间，获得每个最小区间的余票
- 当前余票则为：`min(区间1, 区间2, ...)`

```java
// 假设某车次一共有ABCDE共5个站；现在用户查询B→E的所有车次
// 当用户查询B→E时，系统实时计算：
public int calculateRemainingTickets(String trainId, String fromStation, String toStation) {
    // 1. 获取列车完整路线：A→B→C→D→E
    List<String> route = getTrainRoute(trainId);

    // 2. 找到B和E在路线中的位置
    int fromIndex = route.indexOf("B"); // 索引1
    int toIndex = route.indexOf("E");   // 索引4

    // 3. 计算所有子区间的最小余票
    int minRemaining = Integer.MAX_VALUE;
    for (int i = fromIndex; i < toIndex; i++) {
        String segmentFrom = route.get(i);
        String segmentTo = route.get(i + 1);

        // 查询原子区间 B→C, C→D, D→E 的余票
        int segmentRemaining = getAtomicSegmentRemaining(trainId, segmentFrom, segmentTo);
        minRemaining = Math.min(minRemaining, segmentRemaining);
    }
...
当用户点击“**提交订单**”后，后端会快速创建一个订单，状态设置为`CREATED 已创建`，并将订单ID发给前端；

> 如果用了**预订单**保证幂等性，这里的操作是修改订单的信息和状态；

之后将**订单ID**发到消息队列中，由消费者处理订单后，更新订单的状态；

### 幂等性

#### 分布式锁

原本的项目里是用一个**分布式锁**，key如下：

```
order:lock:{dare}:{train_no}:{useri_d}
```

不启用看门狗机制，不释放锁，过期时间为5min；

在下订单时（发送消息到MQ前），会尝试获取这个锁；也就是说，用户在5min内不可重复购买同一班车；

> 这样的设计有些问题，实际上是可以重复购买同一班车的，只不过在一次订单里不能重复购买；
>

#### UUID

1. 当用户进入下单界面时，前端先请求后端，拿到后端生成的一个**UUID**；
2. 之后用户带上这个UUID下单；
3. 后端拿到UUID后，存入Redis的set；
   1. 如果已存在，则不重复操作，结束方法；
   2. 如果不存在，则插入set，执行下单操作；（TTL设置为**24h**）

如果用户拿到UUID后，如果不下单也没关系，在前端下单前，后端并不会存储UUID；

#### 预订单

1. 当用户进入下单界面时，前端先请求后端，拿到后端生成的预订单ID，即状态为`UNCREATED 未创建`的订单；
2. 之后用户带上这个订单ID下单；
3. 后端拿到订单ID后，查询订单
   1. 若订单状态为`UNCREATED`，则正常执行创建订单逻辑，最后修改状态为`CREATED`；
   2. 若订单状态不为`UNCREATED`，则不重复操作，结束方法；

tip：可以结合上一种方案，Redis里的set存订单ID，先经过Redis的过滤，再经过数据库的过滤；

### 防止消息重复消费

> 这里用了Redis的 set 和MySQL中订单的状态，做双重验证；
>
> 这里应该不需要用到消息的ID，直接用订单ID就可以了；
>
> tip：如果想要更统一处理，可以用outbox + inbox防止消息丢失和重复消费，具体参考后面的**Redis库存预扣减**；

保证了幂等性（防止重复发送消息）后，还需要保证防止重复消费；

- 为了提高可用性，这里可以**先**用Redis的 **set** 来存储所有已经消费过的订单ID，消费拿到订单ID后，去检查Redis
  - 若订单ID存在，则说明已经被消费过了，直接结束方法；
  - 若订单ID不存在，则处理完，把订单ID放入 **set** 中；

- Redis验证成功后，消费者去数据库查询订单

  - 若订单状态是`CREATED 已创建`，则说明还没被消费，执行处理（扣减库存 + 选座 + 修改订单状态）逻辑；

  - 若订单状态是`UNPAID 待支付`，则说明已经被消费过了，直接结束方法；

### 订单状态通知

- 前端不断轮询查询订单状态（指数退避策略），显示“**订单处理中...**”，直到查询得到订单状态为`UNPAID 待支付`，则跳转到支付页面；
- 或者使用WebSocket推送；

### 防止消息丢失

若消息队列挂了，用户可能会一直卡在"**订单处理中...**"，订单得不到处理；

我们可以设置一个**定时任务**来处理未处理的订单

- 时间间隔：假设正常情况下，每个订单在**5s内**处理完成，我们可以把定时任务的间隔设置为**1min**，保证用户体验，如果想要缓解数据库压力，可以设置成**3min**；
- 每次任务执行
  - 查询出所有的状态为`CREATED 已创建`且创建时间距离现在超过1min的订单，可以加上`limit 10`，一次仅处理10条；
  - 对每条数据执行订单处理逻辑（扣减库存 + 选座 + 修改订单状态），这里同样要做**幂等性**判断，防止订单被重复消费；

## Redis库存预扣减

当消费者接收到一个订单后，我们对redis里的库存做扣减；

例子：某车次途经车站ABCDE，用户购买了 **B→E**，则需要扣减 **BC, CD, DE** 这3个区间的余票

- 若有数量 = 0 的票，则扣减失败
  - 消费者将订单状态设置为：`FAILED 失败`;
  - 前端轮询到该状态后，可提示"当前车票已售罄"，不创建订单；

- 若扣减成功，则
  - 消费者将订单状态设置为：`UNPAID 未支付`
  - 前端轮询到该状态后，跳转到支付页面；

### 原子操作

- 由于是扣减多个区间，需要使用**Lua脚本**保证原子操作；
- Redis本身是单线程的，所以我们**不需要加分布式锁**去做扣减操作；
  - 每一条lua脚本看作1条命令，**Redis保证了单条命令的原子性，但不保证多条命令的原子性**；


### 异步更新MySQL

Redis库存扣减后，我们需要**异步更新**MySQL的数据；

有2种方式：开启一个**异步线程** / 使用**消息队列** 异步更新MySQL中的数据；

采用**乐观锁**机制更新MySQL（乐观锁适合读多写少的场景，基本上都会更新成功；

#### MySQL更新失败

最直接的办法是：**回滚数据**

- 检查订单状态和第三方支付平台上的订单状态，若**已支付**，则退款；
- Redis库存加回来，MySQL的座位表的库存信息加回来；

不过这样的话用户体验很糟糕，可以采取别的策略：

- 退款后给一个优惠券，或其他的补偿；

- 多次重试，失败后放入死信队列，**邮件告警**，让工作人员来处理；

#### Outbox 防止消息丢失

Outbox 模式（又称 Transactional Outbox Pattern）是一种常用架构模式，用来解决**数据库操作**与**消息发布**之间的**原子性与一致性问题**。简单来说，就是让**业务数据操作**与**消息发送**在同一事务内写入数据库，然后由独立进程异步读取并转发消息，从而避免分布式事务或消息丢失问题。

创建一张**本地消息表**，字段大致如下，主要关注`status`字段；

```sql
CREATE TABLE outbox (
  id BIGINT PRIMARY KEY, 							-- 主键，唯一标识消息',
  aggregate_type VARCHAR(128) NOT NULL, 			-- 事件所属的聚合类型（业务类型）（如 Order, Inventory）
  aggregate_id VARCHAR(64) NOT NULL, 				-- 聚合实体ID，用于幂等/分区（如订单ID）
  event_type VARCHAR(128) NOT NULL, 				-- 事件类型标识（如 OrderCreated、InventoryReserved）
  payload JSON NOT NULL, 							-- 事件数据负载，存放事件详细信息
  status VARCHAR(32) NOT NULL DEFAULT 'PENDING', 	-- 当前事件状态：PENDING（待发送）、SENT（已发送）、FAILED（发送失败）等
  published_at DATETIME NULL, 						-- 实际发送时间（确认外部已成功消费或排队）
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 消息创建时间
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- 消息最后更新时间
  UNIQUE KEY uq_outbox (aggregate_type, aggregate_id, event_type, created_at), -- 防止重复写入同一个事件
  INDEX idx_status_created (status, created_at)
) COMMENT='事务性 Outbox 表：在业务事务中写入待发送消息，由后台异步发往消息队列';
```

在发送消息时，将**业务操作（即订单表操作）**和**插入outbox表**放在一个**事务**里，设置消息状态为`PENDING 待发送`；

- 若事务提交成功，则发送消息至MQ，并标记消息状态为`SENT 已发送`；


- 若事务提交失败，则标记消息状态为`FAILED发送失败`；


> 这里可以通过事务的回调接口实现；

也需要**定时任务**兜底，可以每1min查一次，查出`PENDING`的消息，发送到MQ；

- 事务成功，则会写入一条 `PENDING` 的数据；
- 如果没发送出去，这条消息可能一直为`PENDING`;
- 如果发送成功，则消息为`SENT`，且已经持久化到MQ，MQ重启后消息不会丢失；
...
[开启数据库事务]  
     ├── 写 业务数据（如订单表）  
     └── 写 outbox 表（事件消息） 对应记录的状态为 PENDING 
     ↓  
[事务提交成功？]  
	 ├── 否 → 标记 outbox 对应记录的状态为 FAILED
     └── 是 → 发送消息至 MQ，并标记 outbox 对应记录的状态为 SENT
                   ↓
         [定时任务查询出状态为 PENDING 的消息，并发送到MQ]
```

#### Inbox 防止重复消费

接收端使用 **Inbox 模式**，是在处理入站消息时保持幂等性（确保“即使多次收到同一消息，也只处理一次”）。其关键是：在开始处理业务逻辑之前，先将消息 ID 存入数据库。如重复插入导致主键冲突，就直接跳过业务逻辑，从而避免重复处理。
...
CREATE TABLE inbox_messages (
  ...
  message_id VARCHAR(128) NOT NULL, 				-- 消息唯一标识符（来自发送端），用于幂等判断
  event_type VARCHAR(128) NOT NULL, 				-- 事件类型，如 OrderCreated、SeatReserved 等
  payload JSON NOT NULL, 							-- 消息完整内容或负载，用于业务处理
  status VARCHAR(32) NOT NULL DEFAULT 'PENDING', 	-- 处理状态：PENDING（待处理）、PROCESSED（已处理）、FAILED（处理失败）',
  processed_at DATETIME NULL, 						-- 实际处理时间戳
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, 	-- 接收消息的时间
  UNIQUE KEY uq_msgid (message_id), 						-- 确保每个 message_id 只处理一次',
  INDEX idx_status_created (status, created_at) 			-- 加速查询 PENDING 消息'
) COMMENT='消费端 Inbox 表：用于幂等消费，确保消息只被处理一次';
```

1. 从 MQ 获取消息（包含 message_id、event_type、payload）
2. 启动数据库事务
3. 尝试插入到 inbox_messages 表：`INSERT … status='PENDING'`
4. 如果插入失败（duplicate key），说明已处理过 → rollback & 忽略
5. 如果插入成功，继续执行业务逻辑
6. 完成后 UPDATE inbox_messages 设置 status='PROCESSED', processed_at = now()
7. 提交事务
...
[消费者收到消息]  
     ↓  
[开启数据库事务]  
     └── 写 inbox 表，对应记录的状态为 PENDING，判断是否插入成功（duplicate key?）
         ├── 否 → 说明已经处理过，忽略
         └── 是 → 继续执行业务逻辑
                     ↓
             最后，将inbox表中对应记录的状态修改为PROCESSED
```
...
用户提交订单后，支付前，前端提示“订单处理中”的时候，消息队列的消费者会执行选座逻辑：

需要先获取到**分布式锁**，锁的key为

```
seat:lock:{date}{train_no}{seat_type}
```

- 用户选座
  - 如果用户选了座位，则遍历座位查找到符合条件的座位
  - 如果没有合适的座位，则降级成顺序选座
- 顺序选座
  - 按顺序选座，有空位则选择

具体来说，每个座位会存一个**位图**，例如6个车站，则初始化为`11111`，1表示该原子区间空闲，用**long**整型存储即可；

从数据库查到列车座位的二维列表`List<List<Integer>>`，每一项是对应座位的位图，例如`list[i][j] = 63（即二进制111111）`；

根据位图做**位运算**，可以求出座位在当前区间内是否已售，具体逻辑如下；

1. 根据用户的订单算出涉及的区间，初始化一个整数**A**为`0`
   1. 遍历每一个区间，对于区间`i`，计算`A |= (1 << i)`；
   2. 遍历结束后得到的**A**形如`0011111000`，`1`的部分是用户下单的区间；
2. 遍历每个座位，拿到每个座位的**位图**，即整型**B_i**，计算`A & B_i == A`，为`true`则座位可选，`false`则不可选；

遍历二维列表是时候即可用上面的逻辑求出是否有符合条件的座位，具体遍历逻辑如下：

1. 假设用户选了二等座DF，即每一行的第4、5个座位；
2. 则我们遍历二维列表的`list[i][3]`，计算是否已售；
3. 若未售出，则检查`list[i][4]`，计算是否已售；
4. 若未售出，则结束循环；若已售出，则遍历下一个`list[i][3]`；

## 支付

前端在轮询时收到后端返回的订单状态为`UNPAID`后，跳转到支付页面；

用**延时队列**延时20分钟：

- 若在时限内支付，则
  - 通过第三方支付平台会调用回调接口，将订单状态修改为`PAID 已支付`

- 消费者会查询订单的状态
  - 若为`PAID 已支付`，则表示已经在时限内支付，则直接结束；
  - 若为`UNPAID 未支付`，还需要去第三方支付平台查询订单状态（防止有可能回调函数触发失败），查询后才确定是未支付还是已支付
    - 若真的未支付，则先调用**关单**接口，防止用户之后做支付操作；
    - 同时修改订单状态为`CANCEL 已取消`；
    - 同时回滚数据，将redis中的对应库存 + 1，也需要异步更新MySQL；

### 应对MQ宕机 — 定时任务

用定时任务每**5min**查出：状态为`UNPAID 未支付`，且更新时间距离现在超过20min的订单，做同样的处理（关单、设置订单状态、回滚）
...
服务端需要管理状态，在分布式系统中，一般用redis统一存储session信息，而不是存储在服务器的内存中；
...
sequenceDiagram
    ...
    用户->>浏览器: 输入用户名/密码
    浏览器->>服务器: POST /login {credentials}
    服务器->>服务器: 验证凭证
    alt 验证成功
        服务器->>存储: 创建 Session (sessionId ↔ 用户信息)
        存储-->>服务器: sessionId
        服务器-->>浏览器: Set-Cookie: sessionId
        浏览器-->>用户: 登录成功页面
    else 验证失败
        服务器-->>浏览器: 登录失败提示
    end

    浏览器->>服务器: 带 Cookie 的请求
    服务器->>存储: 查询 sessionId
    alt session 有效
        存储-->>服务器: 用户信息
        服务器-->>浏览器: 返回所需资源
    else session 无效
        服务器-->>浏览器: 401 或 重定向至登录页
    end
```
...
public class SecureSessionController {
    ...
    public String sessionInfo(HttpServletRequest request) {
        ...
        String username = (auth != null && auth.isAuthenticated()) ? auth.getName() : "匿名";
        return String.format("当前用户：%s，Session ID：%s", username, sessionId);
    }
}
```
...
服务端不需要管理状态，不需要存储token；

与session相比，不具备**强制下线**的功能；

>  具体讲解可以看https://javaguide.cn/system-design/security/jwt-intro.html
...
sequenceDiagram
    ...
    用户->>客户端: 输入用户名/密码
    客户端->>服务器: POST /login {credentials}
    服务器->>服务器: 验证凭证
    alt 验证成功
        服务器-->>客户端: 返回 JWT（包含用户信息、签名、过期时间）
        客户端-->>用户: 登录成功页面
    else 验证失败
        服务器-->>客户端: 登录失败提示
    end

    客户端->>服务器: 请求资源，携带 Authorization: Bearer JWT
    服务器->>服务器: 验证 JWT（签名、有效期）
    alt JWT 有效
        服务器-->>客户端: 资源内容
    else JWT 无效或过期
        服务器-->>客户端: 401 未授权
...
payload.put(JWTPayload.ISSUED_AT, ...); // 设置签发时间
payload.put(JWTPayload.EXPIRES_AT, ...); // 设置过期时间
payload.put(JWTPayload.NOT_BEFORE, ...); // 设置生效时间
payload.put(..., ...) // 其他要保存的数据
...
- 在拦截器中，获取jwt中的信息，存储用户信息到ThreadLocal

- 在网关过滤器中，可以从请求头中获取前端传来的token，验证后放行或返回401未授权


#### reflash token

为了做到无感知刷新token，我们可以给用户一次发2个token：

- **access token**
  - 时间较短（5-30min）
  - 一般存储在内存中或`HttpOnly Cookie`，避免 XSS（前端无法 JS 访问，安全性更高）
  - 保存用户信息，在各种操作时验证用户身份
- **reflash token**
  - 时间较长（5d+）
  - 一般使用 `HttpOnly, Secure, SameSite` 的 Cookie 存储，避免 XSS
  - 前端一般的请求携带**access token**，当前端遇到后端返回的401时，携带**reflash token**，发送到后端刷新token的API；
  - **刷新令牌旋转（Refresh Token Rotation）**：每次用 refresh 换新 token 时，服务端发新的 refresh 并撤销旧的；如果检测到“已用过的 refresh 再次被使用”，应视为泄密并撤销相关会话。
...
@Component
@Data
@ConfigurationProperties(prefix = "hm.cart") // 前缀匹配配置
public class CartProperties {
    private Integer maxAmount; // 对应配置项 hm.cart.maxAmount
}
```

当在 Nacos 上将 `hm.cart.maxAmount` 的值从 10 改为 5 并发布后，`cartProperties` 对象中的 `maxAmount` 属性会自动更新，后续业务逻辑会使用新值；
...
当 `my.config.property` 变更时，这个 Controller Bean 会被重建，`myProperty` 会注入新值；

实现原理：**客户端监听 + 长轮询 （检查到配置变更后主动拉取，并重建Bean）**

1. **初始化与监听注册**：应用启动时，Nacos 客户端会读取配置，连接到 Nacos Server 并获取配置。 同时，它会为相关的 `dataId` 和 `group` 注册一个监听器（Listener）。
2. **长轮询检查变更**：Nacos 客户端内部有一个名为 `ClientWorker` 的组件，它会维护一个定时任务，定期（通过长轮询机制）向 Nacos 服务器发起请求，检查自己关心的配置是否有 MD5 变更。 长轮询的超时时间较长（如30秒），可以在减少频繁请求的同时，相对快速地感知到配置变化。
3. **变更通知与应用刷新**：
   - 当 Nacos 服务器端配置发生变更时，会及时（在长轮询机制下）响应客户端的检查请求；
   - 客户端收到变更通知后，会拉取最新的配置内容；
   - 随后，客户端会发布一个 `RefreshEvent` 等内部事件。 Spring Cloud 上下文监听到此事件后，会触发刷新机制，更新那些被 `@ConfigurationProperties` 标记的 Bean 或重建 `@RefreshScope` 下的 Bean，从而完成配置的热更新；

配置的优先级：`[服务名]-[环境].yaml` > `[服务名].yaml` > 本地配置
...
| 中间件     | 语言生态              | 支持模式                   | 部署架构           | 适用场景              |
| ---------- | --------------------- | -------------------------- | ------------------ | --------------------- |
| **Seata**  | Java                  | AT、TCC、Saga、XA          | 中央 TC 管理       | Java 微服务、强一致性 |
| **DTM**    | Go、Java 等多语言     | Saga、TCC、XA、2PM、Outbox | TM 集群、无独立 TC | 多语言、复杂事务编排  |
| **Hmily**  | Java                  | TCC、TAC                   | 无 TC，集成化设计  | Java、金融级高性能    |
| **DBPack** | 多语言（via sidecar） | AT                         | Sidecar 部署模式   | 云原生、多语言无侵入  |
...
Two-Phase Commit，简称 2PC

1. **准备阶段（Prepare / Vote）**协调者向所有参与者发出 `prepare` 请求；参与者检查本地能否提交（例如能否写入日志并获得锁），若可以则写入“prepared”并回复 `vote-yes`，否则回复 `vote-no`。

2. **提交/回滚阶段（Commit / Abort）**：如果所有参与者都 `vote-yes`，协调者发 `commit`；否则发 `abort`。参与者收到后执行并 ACK；

缺点：

- **阻塞**：一旦参与者回复 `vote-yes` 并进入 prepared 状态，就必须等待协调者的最终决定。若在协调者在这之后宕机（或网络隔离）、且参与者无法判断全局决策，这些参与者就会**挂起**；
  - **关键点**：参与者**无法知道全局事务的情况**，必须一直等待协调者发起决定；（这个重点和3PC比较）



#### 三阶段提交 2PC

Three-Phase Commit，简称3PC

1. **CanCommit**：协调者询问参与者能否提交，参与者回复 `yes/no`（类似 2PC 的准备阶段）；
2. **PreCommit（预提交）**：当协调者收到全部 `yes` 后，先发送 `preCommit`（或 `prepare-to-commit`）给所有参与者。参与者在收到 `preCommit` 后进入 **PRECOMMIT** 状态并 ACK（但此时**还不真正提交**，只是确认都准备好了）；
3. **DoCommit（最终提交）**：当协调者收到所有 PRECOMMIT 的 ACK 后，发送 `doCommit`；参与者真正提交并回复 ACK。若任何阶段出现否定或超时则回滚；



优点：

- 通过引入**预提交**阶段减少了这种不确定性，在单点故障下可以避免无限阻塞，从而改进了恢复能力；
  - **关键点**：参与者在PreCommit阶段了解到了全局事务的情况，若协调者宕机，可以通过超时、选主的方式重新操作（?）

缺点：

- 3PC 要求同步假设（有界网络延迟/有界响应时间）且消息往返更多、延迟更大；在现实有分区或不稳定网络时并不能完美保证；
  - 也就是说：3PC 在**异步网络/分区环境**下并不能保证原先的优点；
...
- **基本原理：**
   XA 模式遵循分布式事务标准（XA 协议），由事务管理器协调各个资源管理器（如数据库）执行**二阶段提交**，从而实现分布式事务的一致性。
- **执行流程**：
   1. TM 开启全局事务，然后调用 RM，RM 向 TC 注册分支事务，然后执行SQL操作
...
- **IP**：按照客户端 IP 限制访问频率；适用广泛、实现简单，但容易被伪造。
- **业务 ID / 用户 ID**：针对用户、设备或客户端维度限流，如按用户 ID 控制请求速率，可实现更细粒度保护。
- **VIP 与普通用户差异化**：对不同用户群体使用不同的限流策略：如 VIP 用户不限流，普通用户受限。
- **实时系统指标**（自适应限流）：依据系统实时状态动态调整限流阈值，包括：
  - 系统 load（如 load1）触发保护
  - CPU 使用率作为阈值
  - 平均响应时间（RT）过高则限流
  - 并发线程数或 QPS 超标自动限流
- **热点参数限流**：如针对某些热门资源（参数）实施更严格的流控，常见于 Sentinel 的“热点限流”功能。
...
| 算法     | 是否支持突发 | 精度   | 实现复杂度 | 典型场景               |
| -------- | ------------ | ------ | ---------- | ---------------------- |
| 固定窗口 | 否           | 较粗略 | 简单       | 流量稳定、非突发场景   |
| 滑动窗口 | 部分支持     | 高     | 中高       | 精准频控、避免窗口突发 |
| 令牌桶   | 是           | 中等   | 中         | API 限流、突发业务保护 |
| 漏桶     | 否           | 中     | 中         | 流量整形、输出平滑     |
...
1. **固定窗口算法（Fixed Window）**
   ...
   - **原理：**
     ...
     - 在每个时间窗口内允许固定数量的请求。当请求数量超过设定值时，后续请求将被拒绝或等待到下一个窗口重新计数。
       - 即针对每个区间计算请求数量，到达区间的右端点后，直接在新的区间重新开始计算；
   ...
   - **适用场景**：
     - 流量相对均匀、不追求精细控制，且对偶发突发不敏感的情况。比如后台任务调度，每小时最多允许1000次操作
...
2. **滑动窗口算法（Sliding Window）**
   ...
   - **适用场景**：
     - 需要限制精确的滑动时间段速率，避免窗口边界突发；对响应精度要求高的系统，例如：登录接口防爆破、金融风控 API 访问频率管控。
...
3. **令牌桶算法（Token Bucket）**
   ...
   - **适用场景**：
     - 适用于业务突发性强，既需限频又需短期支撑高并发的场景。例如 API 门户、文件上传、短时热点抢购等。

4. **滴水/漏桶算法（Leaky Bucket）**
   - **原理：**
     - `均匀速率处理请求`
   
     - 请求进入一个**队列**（桶），以固定速率从队列中“滴出”请求进行处理。
   ...
   - **适用场景**：
     - 适合对输出速率要求稳定的场景，如流媒体推送、视频直播限速、流量整形、网络 QoS 中的带宽控制。
...
### SpringCloud Gateway 令牌桶

SCG框架自带令牌桶的实现，不过需要配合Redis;
  ...
  cloud:
    gateway:
      ...
      - id: products
        ...
        filters:
          - name: RequestRateLimiter  # 框架自带的限流过滤器
            args:
              redis-rate-limiter.replenishRate: 10  # 单位时间（秒）内生成令牌数
              redis-rate-limiter.burstCapacity: 10  # 令牌桶最大容量
              redis-rate-limiter.requestedTokens: 1  # 每次请求消耗的令牌数
              key-resolver: "#{@productKeyResolver}"  # 限流标识规则，例如按用户、IP、路径分组
```
...
限流后的默认行为： 直接**拒绝**请求（**HTTP 429**）

`RequestRateLimiter` 过滤器本身没有内置降级机制，但可以结合 Circuit Breaker 或 Hystrix（现已不推荐） 实现限流后的**降级**处理；

```yaml
# 使用 Circuit Breaker(Resilience4J)
filters:
  - name: CircuitBreaker
    args:
      name: circuitBreakerInstance
      fallbackUri: forward:/fallback/user  # 当接口触发熔断或访问失败时，会转发到降级接口处理
```
...
- Kafka为什么比RocketMQ快
  - RocketMQ的架构参考了Kafka：Topic - Partition(Message Queue)，但主要因为使用的**零拷贝**技术不同，导致吞吐量不同；
  - **RocketMQ**：使用`mmap`零拷贝，该方法返回指针，可用于操作数据，RocketMQ需要对消息做处理；
    - 例如消息二次投递、消息放入DLQ死信队列等；
  - **Kafka**：使用了`sendfile`零拷贝，该方法返回`int`表示操作的字节数，无法对数据做任何处理，但速度快；
...
- 在消息队列中，**消费位点(offset)**很关键
  ...
  - ![rocketmq-consumer-offset](image/rocketmq-consumer-offset.png)
...
定时消息（延迟队列）是指消息发送到broker后，不会立即被消费，而是等待特定时间后再投递给真正的topic；

早期版本仅支持设置**延迟等级**，只能延迟设定好的那几个时间；

RocketMQ 5.0后：生产者可以给定**时间戳** ，即指定时间点，提交任务后会在设定的时间点推送给消费者；默认情况下，延迟消息的最大时间范围为 **24 小时**；

原理：消息发送到服务端，在指定投递时间之前，消息存储在基于时间的存储系统中，不会立即为消息创建索引；
