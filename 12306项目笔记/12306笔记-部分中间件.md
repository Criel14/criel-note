...
### 自定义拦截器（GatewayFilter）

自定义过滤器需要配置的route的filter项中才可以生效
 ...
 */
@Component
public class CountryHeaderGatewayFilter implements GatewayFilter, Ordered {

    @Override
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        // 直接在方法体内实现过滤逻辑，不需要返回 lambda 表达式
        String country = exchange.getRequest().getHeaders().getFirst("Country");
        System.out.println("用户国家信息: " + country);
        return chain.filter(exchange);
    }
...
地址如下：https://github.com/spring-cloud/spring-cloud-gateway/blob/main/spring-cloud-gateway-server-webflux/src/main/java/org/springframework/cloud/gateway/filter/RouteToRequestUrlFilter.java
