...
```
容器启动时 → 构造方法 new Bean 实例 → 注入依赖 → 初始化回调 → 放入 singletonObjects（一级缓存）
```
...
- `String s = "abc";` 创建几个对象？
  ...
  - ⚠ 直接 `=` 则会直接引用常量池的对象，也就是**手写在代码中的字符串**才会进入常量池
...
基于上述的对象创建结论，我们如果在数据库中读取到了非常多重复的字符串，例如一次性读取 `100 w` 条 `User` 信息，其中包含 `gender` 字段，是一个只有 2 种可能的字符串。因为这个字符串不是手写在代码中的，实际上就是 `new` 出来的，所以会创建 100 w 个字符串对象。

`.intern()`方法的核心作用是：**将字符串对象放入 JVM 的字符串常量池中，并返回该字符串在常量池中的唯一引用**。

当我们对字符串都使用这个方法时，就只有 2 个对象被创建了。

然而，如果是不重复的字段，例如手机号，用`.intern()` 方法则会造成字符串常量池被塞爆的情况。

> [!tip]
>
> 实际开发中其实用不到，一般都是框架直接查询出 entity 对象了，而且一般这种数据都会用**枚举**。
...
根据[逃逸分析](#JIT 逃逸分析)，如果逃逸分析确定一个`sychronized`锁对象**不会逃逸出当前线程**，那么 JIT 会**直接消除该同步锁**，因为不存在多线程竞争的可能，加锁是多余的。
...
- ⚠ 在 **JDK 15** 中，偏向锁被**默认禁用并废弃**，因为锁升级时，撤销偏向锁会触发 STW，在现代后端服务中，框架大量使用线程池，导致容易频繁触发，不如直接就用轻量级锁的 CAS 去处理。
...
一个指针（引用）在系统中占用 8 个字节，但是 java 做了优化，压缩到 **4 个字节**，用于解决64位系统下对象指针过大导致的内存浪费和性能下降问题。

4 Byte 的指针，可以表示 `2^32` 个地址，即可以表示 `2^32 Byte = 2^30 * 4 Byte = 4 GB` 的内存空间，而 JVM 中，又设定了 **8 Byte** 为一个块，因此可以表示 `32 GB`的内存空间。

计算机中，每个 Byte（每 8 bit）都有唯一一个地址，JVM 要求 Java 对象的起始内存地址必须是 8 的倍数，而在二进制里，一个数如果是 8 的倍数，它的**最低 3 位一定全是 0**。因此，在使用指针时，将 4 Byte 的指针左移 3 bit，然后高位填充0，就成为了一个 64 位的指针。

> [!tip]
>
> 在现代的服务器中，内存超过 32 GB 很常见，但值得注意的是，如果堆内存设置为了 **33 GB**，那么实际上可以用的空间比设置为 **32 GB** 的时候小，因为此时指针压缩无法开启，全部变为 8 bit 的指针。如果需要更大的内存，则需要开到 40 GB 以上，但仍然会有 GC 扫描变慢，CPU 缓存命中下降等等



## JIT 逃逸分析

逃逸分析是 JIT（JVM 的即时编译器） 在运行时执行的高级优化技术，通过分析**对象的作用域**，来判断一个对象是否会“逃逸”出当前方法或线程。

逃逸分析在 **JDK 1.6** 及以后的版本中默认开启。

- **未逃逸 (No Escape)**：对象的作用域仅限于当前**方法内部**，不会被外部任何方法或线程引用。这是最理想的情况，能触发所有优化。
  - 对于“未逃逸”的对象，JIT编译器会将其“直接分配”在**栈内存**上。随着方法的结束，栈帧被弹出，对象也随之销毁，整个过程**无需GC介入**。
  - ⚠ **标量替换**： 这里的“直接分配”只是形式上，实际上是采用一种叫做**标量替换**的方法：当对象未逃逸时，JIT 可以不创建这个对象，而是将其非聚合的成员变量（如int、long等）拆解成独立的局部变量，直接分配在栈上或CPU 寄存器中。这消除了对象头和内存对齐的开销，执行效率极高。
  - ⚠ **同步消除**：如果逃逸分析确定一个`sychronized`锁对象**不会逃逸出当前线程**，那么 JIT 会**直接消除该同步锁**，因为不存在多线程竞争的可能，加锁是多余的。
- **方法逃逸 (Method Escape)**：对象作为方法返回值返回，或者作为参数传递给了其他方法。编译器无法确定外部方法会如何使用它，因此优化空间有限。
- **线程逃逸 (Thread Escape)**：对象的引用被赋值给一个类变量或实例变量，或者被**传递给了其他线程**。这种对象无法进行逃逸相关的优化，必须保证其线程安全性。
