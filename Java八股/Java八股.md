...
目前的I/0多路复用都是采用的**epoll模式**实现，它会在通知用户进程Socket就绪的同时，把已就绪的Socket写入**用户空间**，不需要挨个遍历Socket来判断是否就绪，提升了性能。

（**epoll** 通过将就绪的文件描述符**socket**直接写入用户空间，避免了每次 I/O 轮询时的遍历过程，从而实现了高效的 I/O 多路复用，是 Linux 系统中构建高性能网络应用的重要技术手段。）
...
跟`HashMap`相关的还有[ConcurrentHashMap](#ConcurrentHashMap)
